...
(defun mix-project-root (&rest opts)
  "Find the root of the current Elixir project."
  (let ((prefer-umbrella (plist-get opts :prefer-umbrella))
        (directory (or (plist-get opts :directory) default-directory)))
    (if prefer-umbrella
        (mix-umbrella-root directory)
      (locate-dominating-file directory "mix.exs"))))
...
;;; Allow project.el to use Mix root as project root
